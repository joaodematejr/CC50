...
void grayscale(int height, int width, RGBTRIPLE image[height][width]) {
  ...
  for (int i = 0; i < width; i++) {
    for (int j = 0; j < height; j++) {
      // averages the color intensity and then applies the same value to all the
      // colors to get gray
      rgbGray = round(
          (image[j][i].rgbtBlue + image[j][i].rgbtGreen + image[j][i].rgbtRed) /
          3.000);
...
void reflect(int height, int width, RGBTRIPLE image[height][width]) {
  ...
  for (int j = 0; j < height; j++) {
    for (int i = 0; i < width / 2; i++) {
      ...
      // swap pixels with the ones on the opposite side of the picture and
      // viceversa
...
void blur(int height, int width, RGBTRIPLE image[height][width]) {
  ...
  for (int i = 0; i < width; i++) {
    for (int j = 0; j < height; j++) {
      ...
      // sums values of the pixel and 8 neighboring ones, skips iteration if it
      // goes outside the pic
      for (int k = -1; k < 2; k++) {
...
void edges(int height, int width, RGBTRIPLE image[height][width]) {
  ...
  for (int i = 0; i < width; i++) {
    for (int j = 0; j < height; j++) {
      ...
      // sums values of the pixel and 8 neighboring ones after applying a
      // modifier, skips iteration if it goes outside the pic
      for (int k = -1; k < 2; k++) {
        if (j + k < 0 || j + k > height - 1) {
          continue;
        }
      ...
      temp[j][i].rgbtBlue =
          limit(round(sqrt(sumBlueX * sumBlueX + sumBlueY * sumBlueY)));
      temp[j][i].rgbtGreen =
          limit(round(sqrt(sumGreenX * sumGreenX + sumGreenY * sumGreenY)));
      temp[j][i].rgbtRed =
          limit(round(sqrt(sumRedX * sumRedX + sumRedY * sumRedY)));
    }
  }
